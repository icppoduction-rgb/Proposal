...
| **`TEST`** | Финальная проверка обобщающей способности на hybrid, cloud и malware-driven данных |
...
> **Роль:** Основной baseline dataset для Host Intrusion Detection Systems.
...
> **Роль:** Основной датасет для enterprise log behavior и multi-stage attack modelling.

**Ссылка:** <https://data.niaid.nih.gov/resources?id=zenodo_5789063>
...
> **Роль:** Validation dataset для CVE-based атак и cross-version проверки.
...
> **Роль:** Проверка enterprise authentication behavior и user-host anomalies.
...
> **Важно:** LANL закрывает слабое место syscall datasets — они хорошо описывают процессы, но плохо описывают поведение пользователей и инфраструктуры.
...
> **Роль:** SOC-level validation на Windows host telemetry.
...
> **Важно:** ADFA и LID-DS больше ориентированы на syscall-level detection. Windows Event Logs добавляют SOC-практичность и связывают research с реальными security operations.
...
> **Роль:** Hybrid validation для host + network detection.
...
> **Важно:** Для proposal с hybrid IDS / exfiltration detection этот dataset особенно важен, потому что он ближе к реальной SOC-архитектуре.
...
> **Роль:** Проверка модели в cloud environment.
...
> **Роль:** Проверка malware-driven host behavior.
...
> **Роль:** Дополнительный baseline для log anomaly detection.
...
**Основная идея:** Host-side часть проекта не должна опираться на один датасет, потому что разные источники покрывают разные уровни поведения:

```
ADFA IDS                           → классический HIDS baseline
LID-DS 2021                        в†' syscall sequence modelling
Maintainable Log Dataset           в†' enterprise multi-stage behavior
LID-DS 2019                        → переносимость внутри syscall-family
LANL                               в†' user / authentication behavior
Windows Event Logs / OTRF          → применимость к SOC telemetry
Unified Host + Network Dataset     в†' hybrid host + network fusion
ISOT Cloud IDS                     в†' cloud environment
Dynamic Malware Analysis Dataset   в†' malware-driven host behavior
```
