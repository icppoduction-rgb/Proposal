# Стратегия DNS-датасетов для проекта обнаружения DNS Exfiltration и DNS Tunneling

---

## 1. Назначение документа

Документ фиксирует набор датасетов для обучения, валидации, тестирования и экспериментов в проекте по обнаружению DNS-эксфильтрации данных и DNS-туннелирования.

**Основная цель** — не смешивать роли датасетов и использовать каждый набор данных по назначению:

| Роль | Назначение |
|:---:|:---|
| **`TRAIN`** | Обучение модели на атаках и нормальном трафике |
| **`VALIDATION`** | Контроль качества модели и ложных срабатываний |
| **`TEST`** | Финальная проверка обобщающей способности модели |
| **`EXPERIMENTS`** | Быстрые эксперименты, прототипирование и проектирование признаков |

---

# 2. Обязательные датасеты

---

## 1. CIC-Bell-DNS-EXF-2021

> **Роль:** Основной датасет для обучения модели атакующему поведению.

**Ссылка:** <https://www.unb.ca/cic/datasets/dns-exf-2021.html>

---

### Описание

CIC-Bell-DNS-EXF-2021 специально создан для обнаружения:

- DNS-эксфильтрации данных;
- DNS-туннелирования;
- легких сценариев атак;
- тяжелых сценариев атак.

**Содержит:**

- легитимный трафик;
- легкие атаки;
- тяжелые атаки;
- признаки с учетом состояния;
- признаки без учета состояния;
- временные признаки, пригодные для моделирования последовательностей.

---

### Подходящие модели

| Тип модели | Применение |
|:---|:---|
| Random Forest | базовая ML-модель |
| XGBoost | сильный baseline для табличных признаков |
| CNN | анализ локальных паттернов признаков |
| LSTM | анализ временных окон и последовательностей |

---

### Почему критичен

> Это ключевой датасет, потому что он:
>
> - напрямую моделирует сценарии DNS-эксфильтрации;
> - содержит разные уровни атак;
> - поддерживает моделирование последовательностей;
> - подходит для LSTM за счет признаков временных окон.

**Использование в проекте:**

```
TRAIN:
- класс атак
- DNS-эксфильтрация
- DNS-туннелирование
```

---

## 2. BCCC-CIC-Bell-DNS-2024

> **Роль:** Финальная проверка модели и исследовательский benchmark.

**Ссылка:** <https://www.yorku.ca/research/bccc/ucs-technical/cybersecurity-datasets-cds/>

---

### Описание

BCCC-CIC-Bell-DNS-2024 объединяет данные из:

- DNS-2021;
- EXF-2021.

Также добавляет расширенное пространство признаков и дополнительные типы атак.

**Содержит:**

- 6 типов атак;
- flow-based признаки;
- application-level признаки;
- 120 признаков;
- расширенное пространство признаков.

---

### Для чего нужен

Используется для **финального тестирования** модели:

- проверка обобщающей способности;
- проверка устойчивости к новым данным;
- проверка работы на разных типах атак;
- выявление переобучения.

> **Важно:** Это самый современный и сложный датасет из списка. Его целесообразно использовать не для основного обучения, а как независимый тестовый benchmark.

**Использование в проекте:**

```
TEST:
- финальная оценка
- проверка обобщения
- проверка переобучения
```

---

## 3. CIC-Bell-DNS-2021

> **Роль:** Baseline нормального DNS-трафика и контроль ложных срабатываний.

**Ссылка:** <https://www.unb.ca/cic/datasets/dns-2021.html>

---

### Описание

CIC-Bell-DNS-2021 содержит около **1 млн доменов**, при этом примерно **99% данных** относятся к легитимному трафику.

**Содержит:**

- легитимный трафик;
- phishing;
- malware;
- spam;
- реалистичное распределение интернет-трафика.

---

### Для чего нужен

Датасет нужен для формирования понимания нормального поведения DNS-трафика.

**Основные задачи:**

- baseline нормального поведения;
- обучение класса benign;
- контроль ложных срабатываний;
- балансировка обучающей выборки;
- добавление real-world benign traffic.

> **Важно:** Без CIC-Bell-DNS-2021 модель будет слишком сильно ориентирована на атаки — это приведет к большому числу ложных срабатываний на реальном DNS-трафике.

**Использование в проекте:**

```
TRAIN:
- класс benign

VALIDATION:
- контроль false positives
- проверка поведения нормального трафика
```

---

# 3. Дополнительные датасеты

---

## 4. Mendeley DNS Exfiltration Dataset

> **Роль:** Проверка реалистичности и устойчивости.

**Ссылка:** <https://data.mendeley.com/datasets/c4n7fckkz3/3>

---

### Описание

Датасет содержит DNS exfiltration traffic и ближе к реальным условиям по сравнению с чисто лабораторными наборами данных.

Используется для проверки устойчивости модели к новым данным.

**Основной сценарий:**

```
TRAIN:
- CIC datasets

TEST:
- Mendeley DNS Exfiltration Dataset
```

**Проверяет:**

- переносимость модели;
- устойчивость к новым паттернам;
- способность работать вне лабораторных условий;
- качество cross-dataset validation.

> Mendeley DNS Exfiltration Dataset показывает, насколько модель применима к данным, которые отличаются от обучающего распределения.

---

## 5. Kaggle DNS Tunneling Dataset

> **Роль:** Быстрые эксперименты и прототипирование.

**Ссылка:** <https://www.kaggle.com/datasets/daumel/dns-tunneling-dataset>

---

### Описание

Датасет является **синтетическим** и содержит DNS tunneling traffic, сгенерированный несколькими tunneling-инструментами.

**Подходит для:**

- быстрых экспериментов;
- проектирования признаков;
- первичного прототипирования;
- data augmentation;
- проверки идей до запуска на основных датасетах.

> **Ограничение:** Датасет синтетический, поэтому его результаты хуже переносятся на реальный трафик. Не стоит использовать как основной источник истины для оценки качества модели.

**Использование в проекте:**

```
EXPERIMENTS:
- быстрые проверки
- проектирование признаков
- augmentation
```

---

# 4. Правильное распределение ролей

| Этап | Датасет | Назначение |
|:---:|:---|:---|
| `TRAIN` | CIC-Bell-DNS-EXF-2021 | обучение на классе атак |
| `TRAIN` | CIC-Bell-DNS-2021 | обучение на benign class |
| `VALIDATION` | часть CIC-Bell-DNS-2021 | контроль false positives |
| `TEST` | BCCC-CIC-Bell-DNS-2024 | финальная проверка и generalization |
| `TEST` | Mendeley DNS Exfiltration Dataset | проверка реалистичности |
| `EXPERIMENTS` | Kaggle DNS Tunneling Dataset | быстрые эксперименты |

---

# 5. Правильная схема обучения

## `TRAIN`

```
CIC-Bell-DNS-EXF-2021
-> класс атак
-> DNS-эксфильтрация
-> DNS-туннелирование
```

```
CIC-Bell-DNS-2021
-> класс benign
-> реальный нормальный DNS-трафик
```

## `VALIDATION`

```
Часть CIC-Bell-DNS-2021
-> validation split
-> контроль false positives
-> настройка threshold
```

## `TEST`

```
BCCC-CIC-Bell-DNS-2024
-> финальный benchmark
-> проверка generalization
-> устойчивость к нескольким типам атак
```

```
Mendeley DNS Exfiltration Dataset
-> cross-dataset validation
-> проверка real-world паттернов
```

## `EXPERIMENTS`

```
Kaggle DNS Tunneling Dataset
-> прототип
-> проектирование признаков
-> быстрое сравнение моделей
```

---

# 6. Итоговая логика использования

**Основная идея:** Модель должна учиться не только распознавать атаки, но и понимать нормальное поведение DNS-трафика.

Поэтому обучение строится так:

```
EXF-2021    → учит модель видеть атаки
DNS-2021    → учит модель понимать нормальный DNS-трафик
BCCC-2024   → проверяет, не переобучилась ли модель
Mendeley    → проверяет применимость к более реалистичным данным
Kaggle      → используется только для быстрых экспериментов
```

---

# 7. Минимальная обязательная конфигурация

Для серьезного проекта минимальный набор должен быть таким:

| Приоритет | Датасет | Статус |
|:---:|:---|:---:|
| 1 | CIC-Bell-DNS-EXF-2021 | **Обязательно** |
| 2 | CIC-Bell-DNS-2021 | **Обязательно** |
| 3 | BCCC-CIC-Bell-DNS-2024 | **Обязательно** |
| 4 | Mendeley DNS Exfiltration Dataset | Желательно |
| 5 | Kaggle DNS Tunneling Dataset | Опционально |

---

# 8. Краткий вывод

## Обязательные

1. **CIC-Bell-DNS-EXF-2021** — основное обучение атак.
2. **CIC-Bell-DNS-2021** — нормальный DNS-трафик и false positives.
3. **BCCC-CIC-Bell-DNS-2024** — финальный benchmark.

## Дополнительные

4. **Mendeley DNS Exfiltration Dataset** — реалистичность и cross-dataset validation.
5. **Kaggle DNS Tunneling Dataset** — быстрые эксперименты и augmentation.

## Финальная схема

```
TRAIN:
  - EXF-2021
  - DNS-2021

VALIDATION:
  - DNS-2021 split

TEST:
  - BCCC-2024
  - Mendeley DNS

EXPERIMENTS:
  - Kaggle DNS
```
