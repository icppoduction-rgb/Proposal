...
- `dns/<role>/<format>.md` and `host/<role>/<format>.md` repeated the same structure for 66 format buckets;
...
| [format_status_matrix.md](format_status_matrix.md) | Single matrix of 66 format buckets with readiness status and key facts. |
...
| --- | ---: | ---: | --- |
| `READY_FOR_FEATURE_EXTRACTION` | 44 | 288871 | The format can be connected to feature extraction after streaming/schema-aware normalization. |
