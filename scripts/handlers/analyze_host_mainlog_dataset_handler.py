...
class HostMainlogContentAnalysisResult:
...
class HostMainlogContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "mainlog"
    ...
    SUMMARY_JSON_FILE = "analysis-host-mainlog-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostMainlogContentAnalysisResult:
        ...
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/mainlog.")
        ...
        docs_ru_path = self.docs_ru_dir / "mainlog.md"
        docs_en_path = self.docs_en_dir / "mainlog.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task20(Analysis of host mainlog dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task20(Analysis of host mainlog dataset files)_report.md"
        ...
        return HostMainlogContentAnalysisResult(
...
## 12. Р’С‹РІРѕРґ
`TRAIN/mainlog` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task20 (Analysis of host mainlog dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/mainlog` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            "## РљР°РєРёРµ С„Р°Р№Р»С‹ Р±С‹Р»Рё РґРѕР±Р°РІР»РµРЅС‹ РёР»Рё РёР·РјРµРЅРµРЅС‹\n"
            "- `scripts/handlers/analyze_host_mainlog_dataset_handler.py`\n"
            "- `manage.py`\n"
            "- `docs/ru/analysis-dataset/host/mainlog.md`\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> mainlog`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task20 (Analysis of host mainlog dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/mainlog` content analysis with RU/EN documentation generation and README index updates.\n\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "mainlog",
                host_counts.get("mainlog", json_summary["scope"]["total_files_count"]),
