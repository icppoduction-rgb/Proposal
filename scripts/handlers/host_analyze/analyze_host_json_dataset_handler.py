...
class HostJSONContentAnalysisResult:
...
class HostJSONContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "json"
    ...
    SUMMARY_JSON_FILE = "analysis-host-json-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostJSONContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/json.")
        ...
        report_ru_path = self.report_ru_dir / "Task11(Analysis of host json dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task11(Analysis of host json dataset files)_report.md"
        ...
        return HostJSONContentAnalysisResult(
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            ...
            "- `PATH_REPORT/ru/stage-one/analysis-dataset/host/Task11(Analysis of host json dataset files)_report.md`\n"
            "- `PATH_REPORT/en/stage-one/analysis-dataset/host/Task11(Analysis of host json dataset files)_report.md`\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            ("json", host_counts.get("json", json_summary["scope"]["total_files_count"]), json_summary["final_status"], "json.md"),
        ]
