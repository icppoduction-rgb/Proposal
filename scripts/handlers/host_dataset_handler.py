...
class HostAnalysisResult:
    """Результат анализа Host-датасетов и сохранения JSON-файлов."""
...
class HostDatasetHandler:
    """Обработчик первого этапа анализа Host-датасетов."""
    ...
    def __init__(self, host_datasets_path: str | Path, temp_data_path: str | Path) -> None:
        self.host_datasets_path_raw = str(host_datasets_path).strip()
        self.host_datasets_path = Path(self.host_datasets_path_raw).expanduser()
        self.temp_data_path = Path(temp_data_path).expanduser()

    def analyze_and_save(self) -> HostAnalysisResult:
        """
        Сканирует Host-датасеты, распределяет пути и файлы по ролям и
        сохраняет результат в `host-path-file.json` и `host-file.json`.
        """
        self._validate_host_root_path()

        paths_by_role_set: dict[str, set[str]] = self._init_role_sets()
        files_by_role_set: dict[str, set[str]] = self._init_role_sets()

        for current_path, files in self._walk_host_files():
        ...
        path_json_file = self.temp_data_path / "host-path-file.json"
        files_json_file = self.temp_data_path / "host-file.json"

        JsonDataManager(path_json_file).write(paths_by_role)
        JsonDataManager(files_json_file).write(files_by_role)

        return HostAnalysisResult(
    ...
    def _validate_host_root_path(self) -> None:
        """Проверяет корректность директории Host-датасетов."""
        if not self.host_datasets_path_raw:
            raise ValueError(
                "Переменная PATH_HOST_DATASETS не задана. "
                "Укажите путь до Host-датасетов в окружении."
            )

        if not self.host_datasets_path.exists():
            raise FileNotFoundError(
                f"Директория Host-датасетов не найдена: {self.host_datasets_path}"
            )

        if not self.host_datasets_path.is_dir():
            raise NotADirectoryError(
                f"PATH_HOST_DATASETS должен указывать на директорию: {self.host_datasets_path}"
            )

    def _walk_host_files(self) -> list[tuple[Path, list[str]]]:
        """Возвращает список директорий Host-датасета, в которых есть файлы."""
        files_by_directory: dict[Path, list[str]] = {}

        for path in self.host_datasets_path.rglob("*"):
            if not path.is_file():
                continue
