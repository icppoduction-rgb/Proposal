...
class HostLoadLogContentAnalysisResult:
...
class HostLoadLogContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "load.log"
    ...
    SUMMARY_JSON_FILE = "analysis-host-load-log-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostLoadLogContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/load.log.")
        ...
        docs_ru_path = self.docs_ru_dir / "load.log.md"
        docs_en_path = self.docs_en_dir / "load.log.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task13(Analysis of host load-log dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task13(Analysis of host load-log dataset files)_report.md"
        ...
        return HostLoadLogContentAnalysisResult(
    ...
    def _build_summary(self, all_paths: list[Path], sampled_paths: list[Path]) -> dict[str, Any]:
        schema_counts = {"json_document": 0, "json_lines": 0, "unparsed": 0}
        ...
        mixed_schema = schema_counts["json_document"] > 0 and schema_counts["json_lines"] > 0
...
## 12. Р’С‹РІРѕРґ
`TRAIN/load.log` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
| Unstable structure | {"yes" if quality["mixed_schema_detected"] else "no"} | should stay stable as Metricbeat JSON-lines |
| Mixed schemas | {"yes" if quality["mixed_schema_detected"] else "no"} | schema branching is not expected here |
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task13 (Analysis of host load-log dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/load.log` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            "## РљР°РєРёРµ С„Р°Р№Р»С‹ Р±С‹Р»Рё РґРѕР±Р°РІР»РµРЅС‹ РёР»Рё РёР·РјРµРЅРµРЅС‹\n"
            "- `scripts/handlers/analyze_host_load_log_dataset_handler.py`\n"
            "- `manage.py`\n"
            "- `docs/ru/analysis-dataset/host/load.log.md`\n"
            "- `docs/en/analysis-dataset/host/load.log.md`\n"
            "- `docs/ru/analysis-dataset/host/README.md`\n"
            "- `docs/en/analysis-dataset/host/README.md`\n"
            "- `report/ru/stage-one/analysis-dataset/host/Task13(Analysis of host load-log dataset files)_report.md`\n"
            "- `report/en/stage-one/analysis-dataset/host/Task13(Analysis of host load-log dataset files)_report.md`\n"
            "- `temp_data/analysis-host-load-log-summary.json`\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> load.log`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task13 (Analysis of host load-log dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/load.log` content analysis with RU/EN documentation generation and README index updates.\n\n"
            ...
            "2. Select bucket `TRAIN -> load.log`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "load.log",
                host_counts.get("load.log", json_summary["scope"]["total_files_count"]),
