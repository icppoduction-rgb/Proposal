...
class DNSSortedPathExportResult:
    """Результат экспорта путей отсортированных DNS-датасетов."""
...
class DNSSortedPathExportHandler:
    ...
    def export_paths(self) -> DNSSortedPathExportResult:
        """Формирует sort-path-dns-file.json из структуры отсортированных DNS-датасетов."""
        ...
        for role in self.ROLE_ORDER:
            role_directory = self.dns_datasets_filter_path / role
        ...
        output_json_path = self.temp_data_path / "sort-path-dns-file.json"
        ...
        JsonDataManager(self.temp_data_path / "sort-path-dns-file-summary.json").write(summary_payload)

        return DNSSortedPathExportResult(
            json_file=str(output_json_path),
            scanned_files_count=scanned_files_count,
    ...
    def _validate_source_root(self) -> None:
        """Проверяет, что директория с отсортированными DNS-датасетами задана и существует."""
        if not self.dns_datasets_filter_path_raw:
            raise ValueError(
                "Переменная PATH_DNS_DATASETS_FILTER не задана. "
                "Укажите путь к директории отсортированных DNS-датасетов."
            )

        if not self.dns_datasets_filter_path.exists():
            raise FileNotFoundError(
                f"Директория PATH_DNS_DATASETS_FILTER не найдена: {self.dns_datasets_filter_path}"
            )
