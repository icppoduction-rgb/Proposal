...
class HostUptimeLogContentAnalysisResult:
...
class HostUptimeLogContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "uptime.log"
    ...
    SUMMARY_JSON_FILE = "analysis-host-uptime-log-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostUptimeLogContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/uptime.log.")
        ...
        docs_ru_path = self.docs_ru_dir / "uptime.log.md"
        docs_en_path = self.docs_en_dir / "uptime.log.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task42(Analysis of host uptime-log dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task42(Analysis of host uptime-log dataset files)_report.md"
        ...
        return HostUptimeLogContentAnalysisResult(
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# Отчёт: Task42 (Analysis of host uptime-log dataset files)\n\n"
            "## Описание задачи\n"
            "Переделан этап анализа `TRAIN/uptime.log` с генерацией RU/EN-документации и обновлением README индексов.\n\n"
            ...
            "- `PATH_REPORT/ru/stage-one/analysis-dataset/host/Task42(Analysis of host uptime-log dataset files)_report.md`\n"
            "- `PATH_REPORT/en/stage-one/analysis-dataset/host/Task42(Analysis of host uptime-log dataset files)_report.md`\n"
            ...
            "2. Выбран bucket `TRAIN -> uptime.log`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task42 (Analysis of host uptime-log dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/uptime.log` content analysis with RU/EN documentation generation and README index updates.\n\n"
            ...
            "2. Select bucket `TRAIN -> uptime.log`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "uptime.log",
                host_counts.get("uptime.log", json_summary["scope"]["total_files_count"]),
