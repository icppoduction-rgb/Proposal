...
class HostJSON1ContentAnalysisResult:
...
class HostJSON1ContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "json-1"
    ...
    SUMMARY_JSON_FILE = "analysis-host-json-1-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostJSON1ContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/json-1.")
        ...
        docs_ru_path = self.docs_ru_dir / "json-1.md"
        docs_en_path = self.docs_en_dir / "json-1.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task12(Analysis of host json-1 dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task12(Analysis of host json-1 dataset files)_report.md"
        ...
        return HostJSON1ContentAnalysisResult(
...
"""
    ...
    def _build_en_markdown(self, summary: dict[str, Any]) -> str:
        ...
        examples = summary["examples"]["paths"]
        return f"""
# Format Analysis: {scope["format"]}-1
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# Отчёт: Task12 (Analysis of host json-1 dataset files)\n\n"
            "## Описание задачи\n"
            "Переделан этап анализа `TRAIN/json-1` с генерацией RU/EN-документации и обновлением README индексов.\n\n"
            ...
            "- `PATH_REPORT/ru/stage-one/analysis-dataset/host/Task12(Analysis of host json-1 dataset files)_report.md`\n"
            "- `PATH_REPORT/en/stage-one/analysis-dataset/host/Task12(Analysis of host json-1 dataset files)_report.md`\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "json-1",
                host_counts.get("json-1", json_summary["scope"]["total_files_count"]),
