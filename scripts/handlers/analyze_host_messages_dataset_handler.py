...
class HostMessagesContentAnalysisResult:
...
class HostMessagesContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    ...
    SUMMARY_JSON_FILE = "analysis-host-messages-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostMessagesContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/messages.")
        ...
        docs_ru_path = self.docs_ru_dir / "messages.md"
        docs_en_path = self.docs_en_dir / "messages.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task25(Analysis of host messages dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task25(Analysis of host messages dataset files)_report.md"
        ...
        return HostMessagesContentAnalysisResult(
...
## 12. Р’С‹РІРѕРґ
`TRAIN/messages` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task25 (Analysis of host messages dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/messages` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> messages`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task25 (Analysis of host messages dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/messages` content analysis with RU/EN documentation generation and README index updates.\n\n"
            ...
            "2. Select bucket `TRAIN -> messages`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "messages",
                host_counts.get("messages", json_summary["scope"]["total_files_count"]),
