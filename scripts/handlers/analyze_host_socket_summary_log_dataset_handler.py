...
class HostSocketSummaryLogContentAnalysisResult:
...
class HostSocketSummaryLogContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "socket.summary.log"
    ...
    SUMMARY_JSON_FILE = "analysis-host-socket-summary-log-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostSocketSummaryLogContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/socket.summary.log.")
        ...
        docs_ru_path = self.docs_ru_dir / "socket.summary.log.md"
        docs_en_path = self.docs_en_dir / "socket.summary.log.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task34(Analysis of host socket-summary-log dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task34(Analysis of host socket-summary-log dataset files)_report.md"
        ...
        return HostSocketSummaryLogContentAnalysisResult(
...
## 12. Р’С‹РІРѕРґ
`TRAIN/socket.summary.log` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task34 (Analysis of host socket-summary-log dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/socket.summary.log` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            "## РљР°РєРёРµ С„Р°Р№Р»С‹ Р±С‹Р»Рё РґРѕР±Р°РІР»РµРЅС‹ РёР»Рё РёР·РјРµРЅРµРЅС‹\n"
            "- `scripts/handlers/analyze_host_socket_summary_log_dataset_handler.py`\n"
            "- `manage.py`\n"
            "- `docs/ru/analysis-dataset/host/socket.summary.log.md`\n"
            "- `docs/en/analysis-dataset/host/socket.summary.log.md`\n"
            "- `docs/ru/analysis-dataset/host/README.md`\n"
            "- `docs/en/analysis-dataset/host/README.md`\n"
            "- `report/ru/stage-one/analysis-dataset/host/Task34(Analysis of host socket-summary-log dataset files)_report.md`\n"
            "- `report/en/stage-one/analysis-dataset/host/Task34(Analysis of host socket-summary-log dataset files)_report.md`\n"
            "- `temp_data/analysis-host-socket-summary-log-summary.json`\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> socket.summary.log`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task34 (Analysis of host socket-summary-log dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/socket.summary.log` content analysis with RU/EN documentation generation and README index updates.\n\n"
            ...
            "2. Select bucket `TRAIN -> socket.summary.log`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "socket.summary.log",
                host_counts.get("socket.summary.log", json_summary["scope"]["total_files_count"]),
