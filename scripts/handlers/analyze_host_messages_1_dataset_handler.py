...
class HostMessages1ContentAnalysisResult:
...
class HostMessages1ContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "messages-1"
    ...
    SUMMARY_JSON_FILE = "analysis-host-messages-1-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostMessages1ContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/messages-1.")
        ...
        docs_ru_path = self.docs_ru_dir / "messages-1.md"
        docs_en_path = self.docs_en_dir / "messages-1.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task26(Analysis of host messages-1 dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task26(Analysis of host messages-1 dataset files)_report.md"
        ...
        return HostMessages1ContentAnalysisResult(
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task26 (Analysis of host messages-1 dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/messages-1` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> messages-1`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task26 (Analysis of host messages-1 dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/messages-1` content analysis with RU/EN documentation generation and README index updates.\n\n"
            ...
            "2. Select bucket `TRAIN -> messages-1`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "messages-1",
                host_counts.get("messages-1", json_summary["scope"]["total_files_count"]),
