...
class HostNetflowIdsContentAnalysisResult:
...
class HostNetflowIdsContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "netflow_ids"
    ...
    SUMMARY_JSON_FILE = "analysis-host-netflow-ids-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostNetflowIdsContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/netflow_ids.")
        ...
        docs_ru_path = self.docs_ru_dir / "netflow_ids.md"
        docs_en_path = self.docs_en_dir / "netflow_ids.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task27(Analysis of host netflow_ids dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task27(Analysis of host netflow_ids dataset files)_report.md"
        ...
        return HostNetflowIdsContentAnalysisResult(
...
## 12. Р’С‹РІРѕРґ
`TRAIN/netflow_ids` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task27 (Analysis of host netflow_ids dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/netflow_ids` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> netflow_ids`.\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "netflow_ids",
                host_counts.get("netflow_ids", json_summary["scope"]["total_files_count"]),
