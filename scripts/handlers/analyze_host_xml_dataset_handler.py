...
class HostXMLContentAnalysisResult:
...
class HostXMLContentAnalysisHandler:
    ROLE_NAME = "TRAIN"
    FORMAT_NAME = "xml"
    ...
    SUMMARY_JSON_FILE = "analysis-host-xml-summary.json"
    ...
    def analyze_and_generate_docs(self) -> HostXMLContentAnalysisResult:
        role_to_formats = self._read_source_json()
        all_paths = self._extract_paths(role_to_formats)
        if not all_paths:
            raise ValueError("No files found in sort-path-host-file.json for TRAIN/xml.")
        ...
        docs_ru_path = self.docs_ru_dir / "xml.md"
        docs_en_path = self.docs_en_dir / "xml.md"
        docs_ru_readme_path = self.docs_ru_dir / "README.md"
        docs_en_readme_path = self.docs_en_dir / "README.md"
        report_ru_path = self.report_ru_dir / "Task43(Analysis of host xml dataset files)_report.md"
        report_en_path = self.report_en_dir / "Task43(Analysis of host xml dataset files)_report.md"
        ...
        return HostXMLContentAnalysisResult(
...
## 12. Р’С‹РІРѕРґ
`TRAIN/xml` РїСЂРёРіРѕРґРµРЅ РґР»СЏ РёР·РІР»РµС‡РµРЅРёСЏ РїСЂРёР·РЅР°РєРѕРІ, РЅРѕ РёР·-Р·Р° СЃРјРµС€РµРЅРёСЏ СЃС…РµРј С‚СЂРµР±СѓРµС‚ РѕС‚РґРµР»СЊРЅРѕРіРѕ parser layer.
"""
...
"""
    ...
    def _build_ru_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        ...
        return (
            "# РћС‚С‡С‘С‚: Task43 (Analysis of host xml dataset files)\n\n"
            "## РћРїРёСЃР°РЅРёРµ Р·Р°РґР°С‡Рё\n"
            "РџРµСЂРµРґРµР»Р°РЅ СЌС‚Р°Рї Р°РЅР°Р»РёР·Р° `TRAIN/xml` СЃ РіРµРЅРµСЂР°С†РёРµР№ RU/EN-РґРѕРєСѓРјРµРЅС‚Р°С†РёРё Рё РѕР±РЅРѕРІР»РµРЅРёРµРј README РёРЅРґРµРєСЃРѕРІ.\n\n"
            "## РљР°РєРёРµ С„Р°Р№Р»С‹ Р±С‹Р»Рё РґРѕР±Р°РІР»РµРЅС‹ РёР»Рё РёР·РјРµРЅРµРЅС‹\n"
            "- `scripts/handlers/analyze_host_xml_dataset_handler.py`\n"
            "- `manage.py`\n"
            "- `docs/ru/analysis-dataset/host/xml.md`\n"
            "- `docs/en/analysis-dataset/host/xml.md`\n"
            "- `docs/ru/analysis-dataset/host/README.md`\n"
            "- `docs/en/analysis-dataset/host/README.md`\n"
            "- `report/ru/stage-one/analysis-dataset/host/Task43(Analysis of host xml dataset files)_report.md`\n"
            "- `report/en/stage-one/analysis-dataset/host/Task43(Analysis of host xml dataset files)_report.md`\n"
            "- `temp_data/analysis-host-xml-summary.json`\n\n"
            ...
            "2. Р’С‹Р±СЂР°РЅ bucket `TRAIN -> xml`.\n"
    ...
    def _build_en_report(self, summary_payload: dict[str, Any], summary_json_path: Path) -> str:
        schema = summary_payload["technical"]["schema_counts"]
        return (
            "# Report: Task43 (Analysis of host xml dataset files)\n\n"
            "## Task description\n"
            "Reworked `TRAIN/xml` content analysis with RU/EN documentation generation and README index updates.\n\n"
    ...
    def _readme_rows(self, json_summary: dict[str, Any]) -> list[tuple[str, int, str, str]]:
        ...
        return [
            ...
            (
                "txt",
                host_counts.get("txt", 0),
                self._load_optional_status(self.temp_data_path / "analysis-host-txt-summary.json"),
                "txt.md",
            ),
            (
                "uptime.log",
                host_counts.get("uptime.log", 0),
                self._load_optional_status(self.temp_data_path / "analysis-host-uptime-log-summary.json"),
                "uptime.log.md",
            ),
            (
                "xml",
                host_counts.get("xml", json_summary["scope"]["total_files_count"]),
