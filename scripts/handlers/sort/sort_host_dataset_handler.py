...
class HostDatasetSortHandler:
    ...
    def sort_and_prepare(self) -> HostDatasetSortResult:
        """
        Читает filter_dataset-host JSON-файлы и формирует структуру:
        PATH_HOST_DATASETS_FILTER/<ROLE>/<format>/...
        """
        self._validate_output_root()

        source_paths_data = JsonDataManager(self.temp_data_path / "filter_dataset-host-path-file.json").read(default={})
        source_files_data = JsonDataManager(self.temp_data_path / "filter_dataset-host-file.json").read(default={})

        self._validate_source_json(source_paths_data, "filter_dataset-host-path-file.json")
        self._validate_source_json(source_files_data, "filter_dataset-host-file.json")
    ...
    def _normalize_file_names(cls, source_files_data: dict[str, Any]) -> dict[str, set[str]]:
        """Нормализует имена файлов из filter_dataset-host-file.json."""
        ...
        for role in cls.ROLE_ORDER:
            ...
            if not isinstance(role_items, list):
                raise ValueError(f"Поле роли '{role}' в filter_dataset-host-file.json должно быть списком.")
    ...
    def _normalize_file_paths(cls, source_paths_data: dict[str, Any]) -> dict[str, list[Path]]:
        """Нормализует пути из filter_dataset-host-path-file.json."""
        normalized: dict[str, list[Path]] = {role: [] for role in cls.ROLE_ORDER}
        for role in cls.ROLE_ORDER:
            role_items = source_paths_data.get(role, [])
            if not isinstance(role_items, list):
                raise ValueError(f"Поле роли '{role}' в filter_dataset-host-path-file.json должно быть списком.")
